'use client';

import { useState } from 'react';
import { shareContent, shareVotingResults, shareVotingSession } from '@/lib/share';
import { useToast } from './ToastProvider';
import Button from './ui/Button';

interface ShareButtonProps {
  type: 'results' | 'session';
  data: any;
  className?: string;
}

export default function ShareButton({ type, data, className }: ShareButtonProps) {
  const [isSharing, setIsSharing] = useState(false);
  const { success, error: showError } = useToast();

  const handleShare = async () => {
    setIsSharing(true);
    try {
      let shareData;
      
      if (type === 'results') {
        shareData = shareVotingResults(data);
      } else {
        shareData = shareVotingSession(data);
      }

      const shared = await shareContent(shareData);
      
      if (shared) {
        success('Content shared successfully!');
      } else {
        showError('Failed to share content');
      }
    } catch (err) {
      showError('An error occurred while sharing');
      console.error('Share error:', err);
    } finally {
      setIsSharing(false);
    }
  };

  return (
    <Button
      onClick={handleShare}
      variant="outline"
      isLoading={isSharing}
      leftIcon={
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
        </svg>
      }
      className={className}
    >
      Share
    </Button>
  );
}

